...
# grab gosu for easy step-down from root (using curl instead of wget)
...
RUN set -eux; \
    # Create bogus database.yml so needed database adapters get installed
    echo '# temporary database.yml to force bundle install to include needed adapters' > ./config/database.yml; \
